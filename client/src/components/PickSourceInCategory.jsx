...
const PickSourceInCategory = ({
  ...
  return (
    <div className="select-as-buttons">
      ...
      {sourceCategory !== null && (
        <div className="source-button-group" role="group" aria-label="Select Source">
          {envelopes.length === 0 && sourceCategory === "envelope" ? (
            <p>
            ...
            <>
              {envelopes.map((envelope) => {
              const isDisabled = envelope.currentAmount < newExpenseAmount || envelope.currentAmount === 0
              return (
                <Button
                  key={envelope.id}
                  type="button"
                  ...
                  onClick={(e) => handleSelectedSourceChange({target: {value: envelope.id}})}
                  disabled={isDisabled}
            ...
            sourceCategory === "savings" && (
              <>
                {Object.values(savings).map((savingsCategory) => {
                  const isDisabled = savingsCategory.currentAmount < newExpenseAmount || savingsCategory.currentAmount === 0
                  return (
                    <Button
                      key={savingsCategory.id}
                      type="button"
                      className={`button button--source button--source--savings ${isDisabled ? "disabled inactive" : selectedSourceId === savingsCategory.id ? "active" : "inactive"}`}
                      onClick={(e) => handleSelectedSourceChange({target: {value: savingsCategory.id}})}
                      disabled={isDisabled}
