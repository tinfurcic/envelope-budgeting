  ...
  try {
    ...
    if (Number(givingEnvelope.budget) >= Number(amount)) {
      updateEnvelope(
        givingEnvelopeId,
        null,
        `${Number(givingEnvelope.budget) - Number(amount)}`,
        null,
      ); // subtracting budget
      updateEnvelope(
        receivingEnvelopeId,
        null,
        `${Number(receivingEnvelope.budget) + Number(amount)}`,
        null,
      ); // adding budget
      return true;
    } else {
      throw new Error(
        `You can transfer at most ${givingEnvelope.budget} dollars (or whatever) from ${givingEnvelope.name}!`,
      );
    }
  } catch (error) {
    console.error(
