...
envelopesRouter.post("/", (req, res, next) => {
  ...
  if (
    Object.hasOwn(newEnvelope, "name") &&
    Object.hasOwn(newEnvelope, "budget") &&
    Object.hasOwn(newEnvelope, "currentAmount")
  ) {
    try {
      const envelope = createEnvelope(
        newEnvelope.name,
        newEnvelope.budget,
        newEnvelope.currentAmount,
      );
...
envelopesRouter.post("/:envelopeId", (req, res, next) => {
  ...
  const envelopeId = req.params.envelopeId;
  const receivedEnvelopeObject = req.body; // hmmmmm
  const envelope = getEnvelopeById(envelopeId);
  if (envelope) {
    updateEnvelope(
      envelopeId,
      receivedEnvelopeObject.name,
      receivedEnvelopeObject.budget,
      receivedEnvelopeObject.currentAmount,
    );
...
envelopesRouter.post("/transfer/:from/:to/:amount", (req, res, next) => {
  ...
  try {
    const isSuccessful = transferBudget(
      givingEnvelopeId,
      receivingEnvelopeId,
      transferAmount,
    );
